...
class SimulationConfig:
    ...
    sync_to_neo4j: bool = False  # Neo4j sync is OPT-IN
...
class FinancialEcosystem:
    ...
    def __init__(self, config: SimulationConfig):
        ...
        self.simulation_id: Optional[str] = None  # Set when running simulation
        ...
        # Initialize GNN Cascade Predictor
        try:
            from app.services.cascade_predictor import CascadePredictorService
            self.cascade_predictor = CascadePredictorService()
            if self.cascade_predictor.is_model_loaded():
                logger.info("GNN cascade predictor initialized with trained model")
            else:
                logger.info("GNN cascade predictor using heuristic fallback")
        except Exception as e:
            logger.warning(f"Failed to initialize cascade predictor: {e}")
    ...
    def step(self) -> SimulationSnapshot:
        ...
        Order of Operations:
        0. GNN Cascade Risk Prediction (before shock)
        1. Stochastic Shock (optional, adjusted by cascade prediction)
        ...
        # Step 0: GNN Cascade Risk Prediction (before shock)
        cascade_prediction = None
        if self.cascade_predictor:
            try:
                # Prepare agent states for prediction
                agent_states_dict = {}
                for agent_id, agent in self.agents.items():
                    agent_states_dict[agent_id] = {
                        'health': agent.compute_health(),
                        'alive': agent.alive,
                        'capital': getattr(agent, 'capital', 0.0),
                        'liquidity': getattr(agent, 'liquidity', 0.0),
                        'npa_ratio': getattr(agent, 'npa_ratio', 0.0),
                        'crar': getattr(agent, 'crar', 10.0),
                        'risk_appetite': getattr(agent, 'risk_appetite', 0.5),
                    }

                cascade_prediction = self.cascade_predictor.predict_cascade_risk(
                    self.network,
                    agent_states_dict
                )
                ...
                # Log prediction
                logger.info(
                    f"Cascade Prediction: {cascade_prediction['risk_label']} "
                    f"(confidence: {cascade_prediction['confidence']:.2f})"
                )

                # Store in global state for API access
                self.global_state['cascade_risk_level'] = cascade_prediction['risk_level']
                self.global_state['cascade_risk_label'] = cascade_prediction['risk_label']
                self.global_state['cascade_confidence'] = cascade_prediction['confidence']
                self.global_state['cascade_probability'] = cascade_prediction['probability']

            except Exception as e:
                logger.warning(f"Cascade prediction failed: {e}")

        # Step 1: Stochastic Shock (potentially adjusted by cascade prediction)
        if self.config.enable_shocks and np.random.random() < self.config.shock_probability:
            # Adjust shock magnitude based on cascade prediction
            shock_magnitude = -0.15
            if cascade_prediction and cascade_prediction['risk_level'] == 2:
                # Reduce shock if system already at high risk
                shock_magnitude = -0.1
                logger.info("Reduced shock magnitude due to high cascade risk")

            shock_event = self.apply_shock(ShockType.SECTOR_CRISIS, magnitude=shock_magnitude)
        ...
        # Step 11: Optional Neo4j sync after timestep
        # TODO: Re-enable when converting step() to async
        # if self.config.sync_to_neo4j and self.simulation_id:
        #     await self._sync_to_neo4j(snapshot)
    ...
    async def run_async(self, steps: int = None, simulation_id: Optional[str] = None) -> List[SimulationSnapshot]:
        """
        Run the simulation for N steps (async version with Neo4j sync support).

        Args:
            steps: Number of steps to run (default: config.max_timesteps)
            simulation_id: Optional simulation ID for Neo4j sync
        ...
        logger.info(f"Starting async simulation run for {steps} steps", simulation_id=simulation_id)
        ...
        for _ in range(steps):
            ...
            snapshot = await self.step_async()
    ...
    def run(self, steps: int = None) -> List[SimulationSnapshot]:
        """
        Run the simulation for N steps (synchronous version, no Neo4j sync).
    ...
    async def step_async(self) -> SimulationSnapshot:
        """
        Execute one time step of the simulation (async version).
        Same as step() but supports async operations like Neo4j sync.
        """
        logger.info(f"========== TIMESTEP {self.timestep} ==========")
    ...
    async def _sync_to_neo4j(self, snapshot: SimulationSnapshot) -> None:
        """
        Sync current snapshot to Neo4j

        Args:
            snapshot: Simulation snapshot to sync
        """
        try:
            from app.services.network_sync import NetworkSyncService

            sync_service = NetworkSyncService()
            await sync_service.sync_snapshot_to_neo4j(
            ...
            logger.debug(f"Synced timestep {snapshot.timestep} to Neo4j")
        except Exception as e:
