...
# WebSocket and narrator service disabled for demo reliability
ws_manager = None
narrate_event_background = None
...
async def step_simulation(sim_id: str, request: StepRequest) -> StepResponse:
    ...
    Now broadcasts real-time updates via WebSocket!
    """
    ...
    try:
        ...
        for step_num in range(request.num_steps):
            ...
            # Broadcast timestep completion via WebSocket
            if ws_manager:
                await ws_manager.broadcast(
                    ...
                    "simulation_step",
                    {
                        ...
                        "system_health": snapshot.global_metrics["system_health"],
                        "alive_agents": snapshot.global_metrics["alive_agents"]
                    }
                )
                
                # Generate LLM narration for this step
            ...
            if user_agent:
                ...
                if risk_decision:
                    ...
                    # Broadcast pending decision via WebSocket
                    if ws_manager:
                        await ws_manager.broadcast(
                            sim_id,
                            "pending_decision",
                        ...
                        # Generate LLM narration for decision
                        if narrate_event_background:
                            narrate_event_background(
                                simulation_id=sim_id,
                                event_type="pending_decision",
                                event_data={
                                    "title": risk_decision['title'],
                                    "context": risk_decision.get('context', ''),
                                    "timestep": ecosystem.timestep
                                },
                                context={
                                    "agent_id": user_agent.agent_id,
...
async def apply_shock(sim_id: str, request: ShockRequest) -> Dict[str, Any]:
    ...
    try:
        ...
        # Broadcast shock event via WebSocket
        if ws_manager:
            await ws_manager.broadcast(
                sim_id,
                "shock_applied",
            ...
            # Generate LLM narration for shock
            if narrate_event_background:
                narrate_event_background(
                    simulation_id=sim_id,
                    ...
                    context={
                        "global_state": ecosystem.global_state,
                        "system_health": ecosystem.global_state.get("system_health", 0)
                    }
                )
...
async def get_dashboard_data():
    """
    Get formatted dashboard data for 3D visualization.
    Returns data from dashboard_data.json file.
    
    This endpoint serves the network visualization data that was generated
    during simulation runs. The data includes nodes (banks, sectors) and
    links (credit exposures) formatted for D3.js force-directed graphs.
    """
    import json
    from pathlib import Path
    
    # Look for dashboard_data.json in backend root
    dashboard_file = Path(__file__).parent.parent.parent.parent / "dashboard_data.json"
    
    if not dashboard_file.exists():
        raise HTTPException(
            status_code=404,
            detail="Dashboard data not available. Run a simulation first to generate network data."
        )
    ...
    try:
        ...
        logger.info(f"Served dashboard data with {len(data.get('d3_network', {}).get('nodes', []))} nodes")
        return data
    
    except json.JSONDecodeError as e:
        logger.error(f"Failed to parse dashboard_data.json: {e}")
        raise HTTPException(
            status_code=500,
            detail="Dashboard data file is corrupted. Please regenerate by running a simulation."
        )
    except Exception as e:
